...
| `topics` | `Topic` | `id` (String) | `id, subjectId, title, masteryLevel` | `database.ts` (l.285) | `topicApi.ts` (`addTopic`) | Not currently implemented | `topicApi.ts` (`updateTopic`) | `topicApi.ts` (`deleteTopic`) | Not currently implemented | `database.ts`, `useAppStore.ts`, `SettingsView.tsx`, `subjectApi.ts` | `src/api/topicApi.ts` (`getTopics`, `addTopic`, `updateTopic`, `deleteTopic`) | `useAppStore.ts` (`useLiveQuery`) | `SettingsView.tsx` (`db.topics.toArray()`) | `putTopic`, `clearTopics` |
...
| `flashcards` | `Flashcard` | `id` (String) | `id, userId, subjectId, topicId, nextReviewDate` | V3 migration `userId` backfill in `database.ts` | `flashcardApi.ts` (`addFlashcard`) | Not currently implemented | `flashcardApi.ts` (`updateFlashcard`) | `flashcardApi.ts` (`deleteFlashcard`) | Not currently implemented | `database.ts`, `useAppStore.ts`, `SettingsView.tsx`, `subjectApi.ts` | `src/api/flashcardApi.ts` (`getFlashcards`, `addFlashcard`, `updateFlashcard`, `deleteFlashcard`) | `useAppStore.ts` (`useLiveQuery`) | `SettingsView.tsx` (`db.flashcards.toArray()`) | `putFlashcard`, `clearFlashcards` |
...
| `goals` | `Goal` | `id` (String) | `id, userId, subjectId, status` | Not currently implemented | `goalApi.ts` (`addGoal`) | Not currently implemented | `goalApi.ts` (`updateGoal`) | `goalApi.ts` (`deleteGoal`) | Not currently implemented | `database.ts`, `useAppStore.ts`, `subjectApi.ts` | `src/api/goalApi.ts` (`getGoals`, `addGoal`, `updateGoal`, `deleteGoal`) | `useAppStore.ts` (`useLiveQuery`) | None | `putGoal`, `clearGoals` |
...
| `statistics` | `Statistic` | `id` (String) | `id, userId, [userId+metricKey+periodStart]` | Not currently implemented | Not currently implemented | Not currently implemented | Not currently implemented | Not currently implemented | Not currently implemented | `database.ts`, `useAppStore.ts` | `src/api/statisticApi.ts` (`getStatistics`) | `useAppStore.ts` (`useLiveQuery`) | None | `addStatistic`, `putStatistic`, `updateStatistic`, `deleteStatistic`, `clearStatistics` |
...
| `user_achievements` | `UserAchievement` | `id` (String) | `id, userId, [userId+achievementId]` | Not currently implemented | Not currently implemented | Not currently implemented | Not currently implemented | Not currently implemented | Not currently implemented | `database.ts`, `useAppStore.ts` | `src/api/achievementApi.ts` (`getUserAchievements`) | `useAppStore.ts` (`useLiveQuery`) | None | `addUserAchievement`, `putUserAchievement`, `updateUserAchievement`, `deleteUserAchievement`, `clearUserAchievements` |
